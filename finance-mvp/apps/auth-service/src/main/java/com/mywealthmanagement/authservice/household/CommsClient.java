...
/**
 * Sends a transactional email/SMS to an EXPLICIT recipient via notification-service's generic
 * internal endpoint ({@code POST /internal/comms/message}, shared {@code X-Internal-Key}). Used to
 * email a household-invite link to someone who may not have an account yet — the same primitive
 * documents-service and business invoicing already use to reach non-users.
 *
 * <p><b>Best-effort.</b> Delivery must never block or fail the invite itself: the raw token is
 * always returned to the owner as a copyable fallback, so a mail hiccup degrades to "share this
 * link yourself" rather than losing the invite. Short timeouts, and the status is surfaced rather
 * than thrown.
 */
@Component
public class CommsClient {
    ...
    public CommsClient(
            @Value("${notification.uri:http://localhost:8088}") String notificationUrl,
