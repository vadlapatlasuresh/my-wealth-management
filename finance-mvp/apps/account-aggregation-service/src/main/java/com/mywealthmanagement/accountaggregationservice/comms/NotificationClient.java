package com.mywealthmanagement.accountaggregationservice.comms;
 ...
 * X-Internal-Key — the same pattern payment reminders and deal alerts use). Posts an
 * in-app notification and asks notification-service to also email/SMS/push the user
 * subject to their preferences. Every failure is swallowed and logged — notifying must
 * never block or fail the underlying action (e.g. linking an account).
 */
@Component
public class NotificationClient {
    ...
    /** Send an in-app + email notification (best-effort). {@code type} tags the in-app row. */
