 ...
 *
 * Secret names map to property keys by '_' -> '-' (e.g. gemini.api_key -> gemini.api-key),
 * which is what the existing ${...} placeholders bind to. Fetched values take priority,
 * so removing GEMINI_API_KEY from the environment makes the service read it from the store.
 *
    ...
    public void postProcessEnvironment(ConfigurableEnvironment env, SpringApplication app) {
        ...
        for (String scope : scopes.split(",")) {
            ...
            try {
                ...
                values.forEach((k, v) -> {
                    String prop = String.valueOf(k).replace('_', '-'); // gemini.api_key -> gemini.api-key
