package com.mywealthmanagement.businessfinancialsservice.business.storage;
...
/**
 * Stores uploaded business documents/images in Google Cloud Storage.
 *
 * <p>Config-gated (mirrors the other integrations): active only when
 * {@code storage.provider=gcs} and {@code gcs.bucket} is set. On the GCP VM the
 * client authenticates via Application Default Credentials (the VM's service
 * account), so no key file is needed. The GCS client is created lazily on first
 * use, so a service with storage disabled — or missing credentials — still starts
 * cleanly and link-based documents keep working.
 */
@Service
public class DocumentStorageService {
    ...
    public Stored upload(Long userId, Long businessId, String filename, String contentType, byte[] bytes) {
        requireEnabled();
        String object = "business/" + userId + "/" + businessId + "/" + UUID.randomUUID() + "-" + sanitize(filename);
    ...
    /** Streams a stored object back (for the authenticated download endpoint). */
