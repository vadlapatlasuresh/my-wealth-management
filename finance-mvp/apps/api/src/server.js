...
app.get("/v1/me/snapshot", authMiddleware, async (req, res) => {
  ...
  const investments = 265930.4;
  const creditCards = accounts
    .filter((a) => a.type === "CREDIT_CARD")
    .reduce((sum, a) => sum + a.balance, 0);
  const loans = 18480;

  const total = cash + investments - creditCards - loans;
...
app.post("/v1/planning/debt-scenarios", authMiddleware, (req, res) => {
  ...
  const base = {
    AVALANCHE: { months_to_debt_free: 22, total_interest_paid: 4120 },
    SNOWBALL: { months_to_debt_free: 25, total_interest_paid: 4580 },
    HYBRID: { months_to_debt_free: 23, total_interest_paid: 4310 }
  };
...
app.get('/v1/real-estate', authMiddleware, async (req, res) => {
  ...
  const items = [
    ...
    },
    {
      ...
      monthly_payment: 4300.5,
      last_valuation: new Date().toISOString(),
      notes: 'Small multifamily near transit'
    }
  ];
  return res.json({ items });
});
...
app.get('/v1/real-estate/:id', authMiddleware, async (req, res) => {
  ...
  const base = {
    ...
    prop_2: {
      ...
      cap_rate: 0.076,
      equity: 560000,
      notes: 'Multifamily; 2 units renovated in 2022.'
    }
  };
